...
export function atlasMethod() {
    ...
    if (navbar.classList.contains('open')) {
        var one = document.getElementsByClassName('artboard'),                       //one 单张元素 
            div = document.getElementsByClassName('section-view');                     //div  容器
            console.log(one)
            console.log(div)
        var colCount: number   //定义列数              parseInt() 函数可解析一个字符串，并返回一个整数。
        var colHeightArry = []                                          //定义列高度数组
        var imgWidth = project.artboards[0].width                          //获取单张宽度
